...
def _generate_queries(claim: Claim, domain: str = "general") -> List[str]:
    ...
    if re.search(r"\b\d+\b", text):
        ...
        # Generic: search with "wikipedia" or "facts"
        queries.append(f"{text} wikipedia")
...
# ------------------------------------------------------------------
# Unified search — TAVILY FIRST, then Serper, then Brave
# ------------------------------------------------------------------
def _search_with_fallback(query: str) -> List[dict]:
    ...
    # Then Serper, then Brave as fallbacks
